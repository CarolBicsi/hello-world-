 ...
 * @param {string} matchUrl 用于请求的Url
...
async function littleWorld(e, matchUrl) {
    ...
    if (initialState) {
        e.reply([
            ...
            '创作者：' + initialState.feeds[0].poster.nick
        ])
    } else {
        return false
    }
...
async function littleWorldRes(resUrl) {
    ...
    if (!response.ok) {
        console.log(`Failed to fetch data from ${resUrl}. Status: ${response.status}`)
    }
    let textData = await response.text()
    const regex = /<script>window\.__INITIAL_STATE__=(.+?)<\/script>/s
    const match = regex.exec(textData)
    if (!match) {
        console.log('Failed to find the initial state data in the response')
    }
 ...
 * 抖音视频处理
 * @param e icqq传递的事件参数e
 * @param {string} matchUrl 发出请求的Url
 * @returns 布尔值或者文件的下载路径
 */
async function douyin(e, matchUrl) {
    ...
    if (data) {
        ...
        //console.log('抖音解析出错，啥原因咱也不知道，可能是接口寄了吧~',)
        return false
    }
 ...
 * @param videoID 视频名字，只在存储路径用到，可常量可变量
 * @param dirPath 文件夹路径
 * @param referer 请求头
 * @returns 布尔值或者文件的下载路径
 */
